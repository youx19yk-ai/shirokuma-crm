...
const CALL_TYPES = ["アポ","決済通話","担当者通話","受付通話","不通","提案完了","決裁","コールのみ"];
...
const DEAL_STATUSES = ["商談中","契約済","審査中","取材予定","取材完了","納品予定","納品完了","入金予定","入金済み"];
...
function EditableUrl({ urlData, onSave, onDelete, canDelete }) {
  ...
  return h("div", { className: "phone-row", style: { cursor: "pointer" }, onClick: function() { setEditing(true); } },
    ...
    h("span", { style: { color: "#94a3b8", fontSize: 10, minWidth: 45 } }, urlData.type || "―"),
    ...
    canDelete ? h("button", { className: "btn btn-ghost btn-sm", style: { color: "#ef4444", padding: "0 4px", fontSize: 10 }, onClick: function(e) { e.stopPropagation(); onDelete(); } }, "削除") : null
  );
}
